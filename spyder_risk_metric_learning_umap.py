...
def biom_of_distances(df1, df2):
    ...
    for i in range(df1.shape[0]):
        for j in range(df2.shape[0]):
            ...
            distance_matrix[i, j] = (df1.iloc[i]- df2.iloc[j])
...
df_distances_AB = pd.DataFrame(biom_of_distances(df[['ABratio' ] ], risky[['ABratio' ] ]),index=df.index, columns=risky.index)
df_distances['average_distance_AB'] = df_distances_AB.mean(axis=1)
df = df.join(df_distances['average_distance_AB'])


df_distances_Tau = pd.DataFrame(biom_of_distances(df[['PTAU181' ] ], risky[['PTAU181' ] ]),index=df.index, columns=risky.index)
df_distances['average_distance_Tau'] = df_distances_AB.mean(axis=1)
df = df.join(df_distances['average_distance_Tau'])

df_distances_NFL = pd.DataFrame(biom_of_distances(df[['NFL' ] ], risky[['NFL' ] ]),index=df.index, columns=risky.index)
df_distances['average_distance_NFL'] = df_distances_AB.mean(axis=1)
df = df.join(df_distances['average_distance_NFL'])



X  = df[['average_distance', 'average_distance_AB', 'average_distance_Tau', 'average_distance_NFL']]


low_risk_condition = (df['ABratio'] > 0.058) & (df['PTAU181'] < 63)
high_risk_condition = (df['ABratio'] <= 0.058) | (df['PTAU181'] >= 63)


df['RiskCategory'] = 'Uncategorized'  # Default value

df.loc[low_risk_condition, 'RiskCategory'] = 0
df.loc[high_risk_condition, 'RiskCategory'] = 1
...
reducer = umap.UMAP(n_neighbors=2, min_dist=0.01, n_components=2, random_state=42)
...
plt.colorbar(boundaries=np.arange(6)-0.5).set_ticks(np.arange(1))
plt.title('UMAP projection of the dataset with 2 clusters')
plt.savefig('/Users/ali/Desktop/Jun24/Alex_Wstein_Risk_model_spline_ADRC/plots/umap_2cluster_biom_and_conn_distance.png', dpi=300)
plt.show()
print(df.head())
...
plt.scatter(X_embedded[:, 0], X_embedded[:, 1], c=y, cmap='viridis', s=50, alpha=0.7)
plt.colorbar(boundaries=np.arange(3)-0.5).set_ticks(np.arange(2))
plt.title('UMAP projection of the binary classification dataset')
plt.xlabel('UMAP Dimension 1')
plt.ylabel('UMAP Dimension 2')
plt.show()

# df['X_embed_bio'] = [a.flatten() for a in X_embedded]


##### bio distance


#df_distances_bio = pd.DataFrame(biom_of_distances(df['X_embed_bio'], risky['X_embed_bio'  ]),index=df.index, columns=risky.index)
...
plt.legend()
plt.savefig('/Users/ali/Desktop/Jun24/Alex_Wstein_Risk_model_spline_ADRC/plots/UNET2.png', dpi=300)
plt.show()
