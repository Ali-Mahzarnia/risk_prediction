...
path_master='new_data_grid-export.csv'
...
####plains
path_connec="/Users/ali/Desktop/Jan24/ADRC/ADRC_connectome_bank/fmri_connectomes/"
...
Ab_path = 'alex-badea_2023-12-05.xlsx'
Ab = read_xlsx(Ab_path)
sum(  datatemp$PTID %in% Ab$PTID )
match(data$PTID , Ab$PTID)
...
for (i in 1:dim(connectivity)[3]) {
  ...
  temp_index=which(datatemp$PTID[i]==(substr(file_list,12,19)))
  temp2 = which(datatemp$PTID[i] ==  Ab$PTID)
  if ((length(temp_index)>0) & (length(temp2)>0) ) 
  { #print(temp_index)
    datatemp$Ab40[i] = Ab$AB40[temp2]
    datatemp$Ab42[i] = Ab$AB42[temp2]
    datatemp$Tau[i] = Ab$TTAU[temp2]
    datatemp$Ab42by40[i] = datatemp$Ab42[i] /datatemp$Ab40[i]
    datatemp$PTAU181[i] = Ab$PTAU181 [temp2]
    datatemp$NFL[i] = Ab$NFL [temp2]
...
index_biom = which(!is.na(response$NFL))
response = response[index_biom, ]
connectivity= connectivity[,,index_biom ]


library(R.matlab)

writeMat(con ="fmri_biom.mat" ,response=response, connectivity=connectivity )
