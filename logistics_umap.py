...
##########
reducer = umap.UMAP(n_neighbors=2, min_dist=0.1, n_components=2, random_state=42, target_metric='categorical')
...
df['X_embed'] = [a.flatten() for a in X_embedded]
...
nca.fit(X_embedded, y)

metric_fun = nca.get_metric()

metric_fun( X_embedded[1],X_embedded[2])
...
# reducer = umap.UMAP(n_neighbors=2, min_dist=0.01, n_components=2, random_state=42, target_metric='categorical')
# X_embedded = reducer.fit_transform(X, y)
...
df_distances = pd.DataFrame(matrix_of_distances(df['X_embed'],risky['X_embed']),index=df.index, columns=risky.index)
